...
public class PostRepositoryWithBagRelationshipsImpl implements PostRepositoryWithBagRelationships {
    ...
    public Optional<Post> fetchBagRelationships(Optional<Post> post) {
        return post.map(this::fetchProducts);
    }
    ...
    public List<Post> fetchBagRelationships(List<Post> posts) {
        return Optional.of(posts).map(this::fetchProducts).orElse(Collections.emptyList());
    }

    Post fetchProducts(Post result) {
        return entityManager
            .createQuery("select post from Post post left join fetch post.products where post.id = :id", Post.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Post> fetchProducts(List<Post> posts) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, posts.size()).forEach(index -> order.put(posts.get(index).getId(), index));
        List<Post> result = entityManager
            .createQuery("select post from Post post left join fetch post.products where post in :posts", Post.class)
